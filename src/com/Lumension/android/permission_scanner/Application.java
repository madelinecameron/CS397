...
public class Application {
	...
	/**
	 * The number of downloads the application has, is usually a range or
	 * number+ so its a string
	 **/
	 ...
	 *            The version code of the application
	 * @param system
	 *            boolean value of if the application is a system app
	 * @param icon
	 *            the application icon
	 */
	public Application(String label, String name, int versionCode,
			boolean system, Drawable icon, int index) {
		this.label = label;
		this.name = name;
	 ...
	 *            The new string value to be added to the permission list
	 */
	public void addPermission(String permissionName) {
		// Make the output more human readable
		String cleanName = permissionName.replace('_', ' ');
		cleanName = cleanName.toLowerCase();
		cleanName = Character.toUpperCase(cleanName.charAt(0))
	 ...
	 * 
	 * @return True if the application declares the android.permissions.camera
	 *         permission
	 */
	...
	public int calculateThreat() {
		...
		for (String p : permissions) {
			ListEntry<Integer> ent = (ListEntry<Integer>) PermissionRiskValueList
		...
		if (this.getRating() != 0) {
			...
			for (int downloadCounter = 0; downloadCounter < downloadCString.length; downloadCounter++) {
				...
				if (downloadCString[downloadCounter] == '0'
						|| downloadCString[downloadCounter] == '1'
						|| downloadCString[downloadCounter] == '2'
						|| downloadCString[downloadCounter] == '3'
						|| downloadCString[downloadCounter] == '4'
						|| downloadCString[downloadCounter] == '5'
						|| downloadCString[downloadCounter] == '6'
						|| downloadCString[downloadCounter] == '7'
						|| downloadCString[downloadCounter] == '8'
						|| downloadCString[downloadCounter] == '9') {
					downloadNumber[downloadCounter - corrector] = downloadCString[downloadCounter];
	...
	/**
	 * Gets the index of the Application within ApplicationList.appList
	 * @return The index of the Application within ApplicationList.appList
	 */
	public int getIndex() {
		return index;
	}
